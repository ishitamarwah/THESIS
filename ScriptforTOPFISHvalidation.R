...
# This script covers the TOPFISH validation analyses
#================================================
...
{
		df_dat<-read.csv(paste0("../",sampleSource,"_RawReadCount/transcript_count_matrix_", sampleType,".csv"),
		                 header = T, stringsAsFactors = F)
		return(df_dat)
}

#-----------------------------------------
# This function reads in and annotates (with HGNC gene symbols corresponding to ensembl transcript indentifiers)
# transcript count files generated with prepDE.py 
#------------------------------------------
...
#----------------------------------------------------
# This function is to get HGNC symbols for a data 
# frame (df_dat) containing ensembl transcript identifiers
#----------------------------------------------------
...
#-------------------------------------------------------------
# Differential expression is performed alphabetically by default i.e. uses the
# alphabetically prior group as baseline. For example,comparison between Fix and UF,  will give transcripts 
# differentially expressed in UF Vs Fix. To reverse this comparison i.e. Fix Vs UF, set reverseComparison = TRUE
#--------------------------------------------------------------
...
#------------------------------------
#
# Compare differential expression across comparisons
#
#------------------------------------

compareDETtotranscriptnumbers<-function(locRObjects,rObj1, rObj2)
{
  # read in R objects containing differential expression information, generated through the function above
  
  fitObject1<-readRDS(paste0(locRObj, rObj1))
  fitObject2<-readRDS(paste0(locRObj, rObj2))
  
 # get numbers of signficant DETs for each comparison
  
  fitObject1_DET <- dim(subset(fitObject1[[4]][[16]], PValue < 0.05))[1]
  fitObject2_DET <- dim(subset(fitObject2[[4]][[16]], PValue < 0.05))[1]
  
  # for each comparison, get number of significant DETs relative to 
  # all transcripts that cleared edgeR filters and were used for DE analysis
  
  print (paste0("No of significant DET ", rObj1," :", fitObject1_DET, " Of ",dim(fitObject1[[4]][[16]])[1]), "total transcripts")
  print (paste0("No of significant DET ", rObj2," :", fitObject2_DET, " Of ",dim(fitObject2[[4]][[16]])[1]), "total transcripts")
...
# <- <- <- <- <- <- <- <- <- <- <- <- <- <-  <- 
#
# END OF FUNCTIONS USED FOR THE FOLLOWING ANALYSES FOR TOPFISH VALIDATION
#
# <- <- <- <- <- <- <- <- <- <- <- <- <- <-  <- 
...
compareDETtotranscriptnumbers(locRObj, "list_BAL_T4M_Vs_BAL_Fix.rds","list_BAL_T4M_Vs_BAL_UF.rds")

DET_BAL_T8MVsFix<-getDET("BAL", "BAL", "T8M", "Fix")
DET_BAL_T8MVsUF<-getDET("BAL", "BAL", "T8M", "UF",TRUE)

# To be done after calculating DET
compareDETtotranscriptnumbers(locRObj, "list_BAL_T8M_Vs_BAL_Fix.rds","list_BAL_T8M_Vs_BAL_UF.rds")


DET_BAL_CD206VsFix<-getDET("BAL", "BAL", "CD206", "Fix")
DET_BAL_CD206VsUF<-getDET("BAL", "BAL", "CD206", "UF",TRUE)

# To be done after calculating DET
compareDETtotranscriptnumbers(locRObj, "list_BAL_CD206_Vs_BAL_Fix.rds","list_BAL_CD206_Vs_BAL_UF.rds")
...
# to be done after calculating DET
compareDETtotranscriptnumbers(locRObj, "list_Blood_T4M_Vs_Blood_Fix.rds","list_Blood_T4M_Vs_Blood_UF.rds")

DET_Blood_T8MVsFix<-getDET("Blood", "Blood", "T8M", "Fix")
DET_Blood_T8MVsUF<-getDET("Blood", "Blood", "T8M", "UF",TRUE)

# to be done after calculating DET
compareDETtotranscriptnumbers(locRObj, "list_Blood_T8M_Vs_Blood_Fix.rds","list_Blood_T8M_Vs_Blood_UF.rds")
